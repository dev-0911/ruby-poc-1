...
class Api::RepositoriesController < Api::BaseController
  ...
  # @parameter q             [string]  Search and filter the Repository data using Lucene syntax.
  #
  # @response_type [array<Repository>]
  #
  ...
  #
  # @path [PUT] /repositories/{id}
  # @summary Update a Repository
  #
  # @parameter owner_id                  [uuid]                                       ID of the User that's the owner of this repository.
  # @parameter in_audit_scope            [boolean]                                    Flag to indicate if this Repository is in scope.
  # @parameter out_of_audit_scope_reason [enum<{Device::OUT_OF_AUDIT_SCOPE_REASONS}>] Out of scope reason if the repository is not in scope.
  #
  # @response_type [Repository]
  #
  # @response 404 Resource not found
  ...
  private def set_update_interaction_variables(valid_params:)
    ...
    variables[:repository_id] = params[:id]
